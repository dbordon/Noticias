...
{
    for (curid in akeeba.Configuration.passwordFields)
    {
    ...
    akeeba.System.doAjax(
        ...
        function (data)
        {
            ...
            if (!empty(data.breadcrumbs))
            {
                ...
                akeeba.Configuration.FtpBrowser.addCrumb(akeeba.System.Text._("COM_AKEEBA_FILEFILTERS_LABEL_UIROOT"),
                    "/",
                    elBreadCrumbs
                );
            ...
            if (!empty(data.list))
            {
                ...
                for (i = 0; i < data.list.length; i++)
                {
                    ...
                    akeeba.Configuration.FtpBrowser.createLink(data.directory + "/" + item, item,
                        elFTPBrowserFolderList
                    );
                }
            }
        },
    ...
    akeeba.System.doAjax(
        ...
        function (data)
        {
            ...
            if (!empty(data.breadcrumbs))
            {
                ...
                akeeba.Configuration.FtpBrowser.addCrumb(akeeba.System.Text._("COM_AKEEBA_FILEFILTERS_LABEL_UIROOT"),
                    "/",
                    elSFTPCrumbs, akeeba.Configuration.SftpBrowser
                );
                ...
                for (i = 0; i < data.breadcrumbs.length; i++)
                {
                    ...
                    akeeba.Configuration.FtpBrowser.addCrumb(crumb, relativePath, elSFTPCrumbs,
                        akeeba.Configuration.SftpBrowser
                    );
                }
            }
            ...
            if (!empty(data.list))
            {
                ...
                for (i = 0; i < data.list.length; i++)
                {
                    ...
                    akeeba.Configuration.FtpBrowser.createLink(data.directory + "/" + item, item,
                        elSFTPBrowserFolderList, akeeba.Configuration.SftpBrowser
                    );
                }
            }
        },
...
akeeba.Configuration.enablePopoverFor = function (el)
{
    ...
    akeeba.Loader.add('akeeba.Tooltip', function () {
...
akeeba.Configuration.parseGuiData = function (data, rootnode)
{
    ...
    for (var headertext in data)
    {
        ...
        for (var config_key in groupdef)
        {
            ...
            {
                akeeba.Configuration.GUI.renderOptionTypeUnknown(current_id, defdata, controlWrapper, row_div,
                    container
                );
            }
        }
 ...
 * @param   {Element} container       The element which contains the row_div (option group container)
 */
akeeba.Configuration.GUI.renderOptionTypeSeparator =
 ...
 * @param   {Element} container       The element which contains the row_div (option group container)
 */
akeeba.Configuration.GUI.renderOptionTypeInstaller =
    function (current_id, defdata, controlWrapper, row_div, container)
    {
    ...
    akeeba.System.addEventListener(editor, "change", function (e)
    {
        ...
        if (elLegend instanceof Element)
        {
            elLegend.insertAdjacentHTML(
                "afterend",
                "<p class=\"akeeba-block--information\">" + enginedef.information.description + "</p>"
            );
        }
 ...
 * @param   {Element} container       The element which contains the row_div (option group container)
 */
akeeba.Configuration.GUI.renderOptionTypeBrowsedir =
    function (current_id, defdata, controlWrapper, row_div, container)
    {
...
akeeba.Configuration.onFormSubmit = function (event)
{
    ...
    let task = document.forms['adminForm']['task'].value;

    if (task !== 'apply' && task !== 'save' && task !== 'savenew')
    {
        return true;
    }
    ...
    akeeba.Configuration.submitJsonForm(
        JSON.stringify(Object.fromEntries(new FormData(adminForm)))
    );
};

akeeba.Configuration.submitJsonForm = async function (json)
{
...
{
    akeeba.Configuration.FtpTest.testConnection("engine.archiver.directftpcurl.ftp_test",
...
{
    akeeba.Configuration.SftpTest.testConnection("engine.archiver.directsftpcurl.sftp_test",
...
akeeba_onedrivebusiness_refreshdrives = function (params)
{
    ...
    if (typeof params["engine.postproc.onedrivebusiness.drive"] === "undefined")
    {
        params["engine.postproc.onedrivebusiness.drive"] = {
            "default": document.getElementById("var[engine.postproc.onedrivebusiness.drive]").value
        };
    }
    ...
    var data = {
        ...
        params: {
            "engine.postproc.onedrivebusiness.access_token":  document.getElementById(
                "var[engine.postproc.onedrivebusiness.access_token]").value,
            "engine.postproc.onedrivebusiness.refresh_token": document.getElementById(
                "var[engine.postproc.onedrivebusiness.refresh_token]").value
        }
    };
...
akeeba.Loader.add(['akeeba.UserInterfaceCommon', 'akeeba.System', 'akeeba.Ajax'], function ()
{
    ...
    setTimeout(function ()
    {
        ...
        akeeba.Configuration.parseConfigData(akeeba.System.getOptions("akeeba.Configuration.GUIData", {}));
        ...
        if (adminForm) {
            adminForm.addEventListener('submit', akeeba.Configuration.onFormSubmit);
        }
    }, 10);
});